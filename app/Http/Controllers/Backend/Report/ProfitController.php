...
class ProfitController extends Controller
{
    ...
    public function monthlyProfitDatewais(Request $request)
    {
        ...
        $html['thsource']  = '<th>Pagamentos de Alunos</th>';
        $html['thsource'] .= '<th>Outras Despesas</th>';
        $html['thsource'] .= '<th>Salários de Colaboradores</th>';
        ...
        $html['tdsource'] .= '<td>';
        $html['tdsource'] .= '<a class="btn btn-sm btn-"'.$color.'"title="PDF"
                                target="_blanks" href="' . route('report.profit.pdf') .
                                '?start_date='.$sdate.'&end_date='.$edate.'">Relatório</a>';
        $html['tdsource'] .= '</td>';
    ...
    public function monthlyProfitPdf(Request $request)
    {
        ...
        $data['edate'] = date('Y-m-d', strtotime($request->start_date));
